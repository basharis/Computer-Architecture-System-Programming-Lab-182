...
void handle_ls(cmd_line * cmd) {
    char buffer[BUFFER_SIZE];
    memset(buffer, 0, BUFFER_SIZE);
    int bytes_recv, bytes_sent;
    char * ls = "ls";
    if ((bytes_sent = send(_client_state.sock_fd, ls, strlen(ls), 0)) < 0){
        perror("send failed");
        return;
    }
    if ((bytes_recv = recv(_client_state.sock_fd, buffer, 3, 0)) < 0){
        perror("recv failed");
        return;
    }
    if (strncmp(buffer, "ok ", 3) == 0){
        if ((bytes_recv = recv(_client_state.sock_fd, buffer, LS_RESP_SIZE, 0)) < 0){
            perror("recv failed");
            return;
        }
        printf("%s\n", buffer);
        ...
        fprintf(stderr, "Server Error: %s", buffer);
        handle_bye();
    }
    else {
        fprintf(stderr, "Server sent incorrect response.\n");
    }
}
